...
export default async (identities: string[]): Promise<TwitchUser[]> => {
  ...
  if (usersToLookup.length > 0) logger.debug(`Got twitch data on ${twitchResults.length} users`);
  ...
  if (expiredDocs.length > 0) {
    ...
    logger.debug(`Removed ${usersToLookup.length} expired users from the database`);
  }
