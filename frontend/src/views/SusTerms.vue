<template>
  <v-row no-gutters class="pa-3" align="center">
    ...
    <v-spacer/>
    <v-btn @click="home" rounded="false" color="primary" class="mr-2" icon size="small">
  ...
  <v-dialog v-model="addDialog">
    <v-card theme="light" min-width="400">
      ...
      <v-card-text>
        <v-text-field v-model="selectedTerm.name" flat autofocus placeholder="New Term" single-line/>
      </v-card-text>
      <v-card-actions>
        <v-spacer/>
  ...
  <v-dialog v-model="deleteDialog">
    <v-card theme="light"  min-width="400">
      ...
      <v-card-text>
        Are you sure you want to delete the term: <span class="bold">{{ selectedTerm.name }}</span>
      </v-card-text>
      <v-card-actions>
        <v-spacer/>
...
export default {
  ...
  methods: {
    ...
    confirmAdd() {
      api.post('/terms', this.selectedTerm)
        .then(({data}) => {
    ...
    confirmRemove() {
      api.delete(`/terms/${this.selectedTerm._id}`)
        .then(() => {
          const index = this.terms.findIndex(x => x._id === this.selectedTerm._id)
    ...
    confirmEdit() {
      api.put(`/terms/${this.selectedTerm._id}`, this.selectedTerm)
        .then(() => {
          const index = this.terms.findIndex(x => x._id === this.selectedTerm._id)
